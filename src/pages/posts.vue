<template>
  <div>Posts</div>
  <h1 id="table">
    Lorem ipsum dolor sit amet consectetur adipisicing elit. Eveniet dolores
    numquam, officia nisi rem cumque animi beatae iure quis, adipisci, ullam
    illo eos tempore. Natus voluptatem ipsa obcaecati molestiae incidunt ut
    magnam similique dolorem. Aliquid officiis recusandae et, nihil
    necessitatibus aperiam molestias expedita voluptas? Aut debitis
    consequuntur, ea sunt rerum ad itaque corrupti aperiam maiores deleniti sed,
    veritatis voluptatem aliquid! Sed velit dicta a quos, illo possimus pariatur
    harum obcaecati provident ratione blanditiis inventore rerum, officiis eum.
    Ipsam reiciendis, officia necessitatibus, accusamus corrupti reprehenderit
    beatae minus laudantium perferendis vitae, sit quis nobis nesciunt enim vero
    deleniti dolorem itaque eveniet consequuntur!
  </h1>
</template>
<script>
export default {
  data: () => ({}),
  mounted() {},
};
</script>
<style lang="">
</style>
